...
<table border="1" cellspacing="0">
    <thead>
        <!-- Baris 1: No, Nama, Izin Meninggalkan Kelas, Total -->
        <tr>
            <th rowspan="3">No</th>
            <th rowspan="3">Nama</th>
            <th colspan="24">Izin Meninggalkan Kelas</th>
            <th rowspan="3">Total</th>
        </tr>

        <!-- Baris 2: Keperluan Pribadi & Sekolah -->
        <tr>
            <th colspan="12">Keperluan Pribadi</th>
            <th colspan="12">Keperluan Sekolah</th>
        </tr>

        <!-- Baris 3: Bulan JAN - DES -->
        <tr>
            @for($i = 1; $i <= 12; $i++)
    ...
    <tbody>
        @foreach($izin_kelas as $index => $siswa)
            <tr>
                <td>{{ $index + 1 }}</td>
                ...
                <!-- Kolom Pribadi JAN - DES -->
                @foreach($siswa['izin_bulan'] as $bulan)
                    <td>{{ $bulan['pribadi'] }}</td>
                    @php $grandTotal += $bulan['pribadi']; @endphp
                @endforeach

                <!-- Kolom Sekolah JAN - DES -->
                @foreach($siswa['izin_bulan'] as $bulan)
                    <td>{{ $bulan['sekolah'] }}</td>
                    @php $grandTotal += $bulan['sekolah']; @endphp
                @endforeach
